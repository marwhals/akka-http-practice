...
object LowLevelAPI extends App {
  ...
  val httpAsyncConnectionHandler = Sink.foreach[IncomingConnection] { connection =>
    connection.handleWithAsyncHandler(asyncRequestHandler) // async may help with parallelism
  }
  ...
  /**
   * Method 2: serve back HTTP response asynchronously --- turn sync into async
   */
